  ...
  >
    <div class="md:hidden flex flex-row bg-cardBlue mb-8 p-5 sidebarResp">
      <ul class="flex flex-row">
        <img class="w-12 ml-0 mr-8" :src="logo" />
        <div class="overflow-x-scroll flex flex-row w-screen px-8">
          <li
            ...
            @click="changeCategory(category.cat), (activeIndex = index)"
    ...
    <div
      class="h-full px-2 py-10 overflow-y-auto bg-cardBlue dark:bg-gray-800 md:flex hidden overflow-x-hidden"
    >
      <ul class="space-y-2">
        <li
          v-for="(category, index) in categories"
      ...
      <img class="flex mr-2 ml-2 absolute bottom-2 w-16" :src="logo" />
    </div>
  </aside>
</template>
